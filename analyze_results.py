...
def write_sixcell_tex(outdir, variant="kuairec_first_average", lamb=0.01,
    ...
    if path is None:
        # small is the paper protocol and keeps the canonical name; big gets a
        # distinct suffix so running both protos cannot overwrite one table.
        proto_tag = "" if proto == "small" else f"_{proto}"
        path = os.path.join(SCRIPT_DIR,
            f"sixcell_ablation_{variant.replace('kuairec_','')}{proto_tag}.tex")
...
# =============================================================================
# gamma_order (gamma_o) training-loss-weight sweep table
# =============================================================================
def write_gamma_sweep_tex(variant="kuairec_first_average", lamb=0.01,
    ...
    for fam_label, prefix in families:
        rows = []
        for gamma, dirsuf in grid:
            fpath = os.path.join(SCRIPT_DIR, f"{prefix}{dirsuf}_{variant}", fname)
            data = parse_dict_result(fpath)
            if data is None:
                missing.append(fpath)
            rows.append((gamma, data))
        panels.append((fam_label, rows))

    if missing:
        print(f"\n⚠  gamma_o sweep: MISSING {len(missing)} result files. "
              f"Run CUDA_VISIBLE_DEVICES=0 bash eval_consecgamma_grid_firstavg.sh. "
              f"\n   Example missing: {missing[0]}")
    ...
    out_lines = [
        r"% gamma_o adjacent-order training-loss-weight sweep (separately trained dense ckpts)",
        r"% Generated by analyze_results.py --gamma_sweep",
        r"% Variant: " + variant + r"   lambda=" + str(lamb) +
            r"   lambda_c=0 (score penalty off)   proto=" + proto,
        r"% CS@20 and MaxRun@20 are lower-is-better.",
        r"\begin{table*}[t]",
        r"\centering",
        r"\small",
        r"\caption{Sensitivity to the adjacent-order training weight $\gamma_o$ on \emph{" +
            variant.replace("kuairec_","").replace("_","-") +
            r"} (KuaiRec, $\lambda=" + str(lamb) + r"$, $\lambda_c=0$, " + proto +
            r"-matrix protocol). Each row is a separately trained dense checkpoint; "
            r"$\gamma_o$ weights $\mathcal{L}_{order}$ and $0.01$ is the default. "
            r"CS@20 is the mean adjacent-item similarity and MaxRun@20 the longest "
            r"contiguous run of mutually similar items (both lower is better). Best "
            r"per metric within each family panel is \textbf{bold}.}",
        r"\label{tab:gamma_order_sweep}",
        r"\resizebox{\textwidth}{!}{%",
    ...
    for f_idx, (fam_label, rows) in enumerate(panels):
        # best (bold) per metric over the present rows of THIS panel
        best = {}
        ...
        out_lines.append(r"\multicolumn{" + str(len(metric_cols) + 1) +
                         r"}{l}{\textbf{" + fam_label + r"}} \\")
        for gamma, data in rows:
            cells = []
            for col_idx, (mc, mkey) in enumerate(metric_cols):
                ...
                if v is not None and col_idx in best and abs(v - best[col_idx]) < 1e-12:
                    s = r"\textbf{" + s + r"}"
                cells.append(s)
            out_lines.append(f"{gamma} & " + " & ".join(cells) + r" \\")
        if f_idx < len(panels) - 1:
    ...
    # Console summary
    print("\n" + "=" * 110)
    print(f"GAMMA_o SWEEP (variant={variant}, lambda={lamb}, lambda_c=0, proto={proto})")
    print("=" * 110)
    header = f"{'family':<12} {'gamma_o':>8}  " + " ".join(f"{mc[0]:>10}" for mc in metric_cols)
    print(header)
    print("-" * len(header))
    for fam_label, rows in panels:
        for gamma, data in rows:
            vals = " ".join(
                f"{get_metric(data, m):>10.4f}" if get_metric(data, m) is not None
                else f"{'MISS':>10}" for _, m in metric_cols)
            print(f"{fam_label:<12} {gamma:>8}  {vals}")
    print("=" * 110)
...
# =============================================================================
# T soft-selection temperature sweep table (fixed L_order)
# =============================================================================
def write_softoT_sweep_tex(variant="kuairec_first_average", gamma=0.01,
                           lamb=0.01, proto="small", path=None):
    """Generate the T (soft-selection temperature) sweep LaTeX table.

    Reads test_result{,_small}_softoT_<tag>.txt produced by
    eval_softoT_grid_firstavg.sh (checkpoints from
    train_softoT_grid_firstavg.sh). Every row is a SEPARATELY TRAINED dense
    checkpoint at fixed lambda=0.01 and fixed gamma_o=0.01, decoded
    identically with the score penalty off (lambda_c=0). T sharpens the
    loss's soft item selection pi_s = Q_s^(1/T)/Z (T=1 = the decoder's own
    mixture distribution). T is a TRAINING hyperparameter.

      T      checkpoint dir suffix (after save_pt_{notype_}dense_)   tag
      2.0    lamb001_softo001_T2                                      T2
      1.0    lamb001_softo001                                         T1
      0.5    lamb001_softo001_T05                                     T05
      0.25   lamb001_softo001_T025                                    T025
      0.1    lamb001_softo001_T01                                     T01

    Two family panels (PACER / TRIER_-t); best per metric bold (lower CS/MaxRun).
    """
    families = [
        ("PACER",            "save_pt_dense_"),
        (r"TRIER$_{-t}$",    "save_pt_notype_dense_"),
    ...
    for fam_label, prefix in families:
        rows = []
        for tval, dirsuf, tag in grid:
            fpath = os.path.join(SCRIPT_DIR, f"{prefix}{dirsuf}_{variant}",
                                 f"test_result{suffix}_softoT_{tag}.txt")
            data = parse_dict_result(fpath)
            if data is None:
                missing.append(fpath)
            rows.append((tval, data))
        panels.append((fam_label, rows))

    if missing:
        print(f"\n  T sweep: MISSING {len(missing)} result files. "
              f"Run CUDA_VISIBLE_DEVICES=0 bash eval_softoT_grid_firstavg.sh. "
              f"\n   Example missing: {missing[0]}")

    def fmt(v):
        return "\\textemdash" if v is None else f"{v:.4f}"

    pretty_var = variant.replace("kuairec_", "").replace("_", "-")
    out_lines = [
        r"% T soft-selection temperature sweep for L_order (separately trained dense ckpts)",
        r"% Generated by analyze_results.py --T_sweep",
        r"% Variant: " + variant + r"   gamma_o=" + str(gamma) +
            r"   lambda=" + str(lamb) + r"   lambda_c=0   proto=" + proto,
        r"\begin{table*}[t]",
        r"\centering",
        r"\small",
        r"\caption{Sensitivity to the soft-selection temperature $T$ in "
            r"$\pi_s(j) \propto Q_s(j)^{1/T}$ of $\mathcal{L}_{order}$ on \emph{" +
            pretty_var +
            r"} (KuaiRec, $\gamma_o=" + str(gamma) + r"$, $\lambda=" + str(lamb) +
            r"$, $\lambda_c=0$, " + proto +
            r"-matrix protocol). Each row is a separately trained dense "
            r"checkpoint, decoded identically; $T=1$ is the decoder's own "
            r"mixture distribution. Best per metric within each family panel "
            r"is \textbf{bold}.}",
        r"\label{tab:softoT_sweep}",
        r"\resizebox{\textwidth}{!}{%",
        r"\begin{tabular}{l " + "r"*len(metric_cols) + r"}",
        r"\toprule",
        r"\textbf{$T$} & "
        ...
        for tval, data in rows:
            ...
            for col_idx, (mc, mkey) in enumerate(metric_cols):
                ...
                if v is not None and col_idx in best and abs(v - best[col_idx]) < 1e-12:
                    s = r"\textbf{" + s + r"}"
                cells.append(s)
            out_lines.append(f"{tval} & " + " & ".join(cells) + r" \\")
        if f_idx < len(panels) - 1:
            out_lines.append(r"\midrule")
    out_lines += [
        r"\bottomrule",
    ...
    if path is None:
        path = os.path.join(SCRIPT_DIR,
            f"softoT_sweep_{variant.replace('kuairec_','')}_{proto}.tex")
    with open(path, "w") as f:
        f.write(table_str + "\n")
    print(f"T sweep table written: {path}")

    print("\n" + "=" * 110)
    print(f"T SWEEP (variant={variant}, gamma_o={gamma}, lambda={lamb}, lambda_c=0, proto={proto})")
    print("=" * 110)
    header = f"{'family':<12} {'T':>6}  " + " ".join(f"{mc[0]:>10}" for mc in metric_cols)
    print(header)
    print("-" * len(header))
    for fam_label, rows in panels:
        for tval, data in rows:
            ...
            print(f"{fam_label:<12} {tval:>6}  {vals}")
    print("=" * 110)
...
# =============================================================================
# lambda_c hard INFERENCE-penalty sweep table (no retraining)
# =============================================================================
def write_penalty_sweep_tex(variant="kuairec_first_average", lamb=0.01,
                            proto="small", path=None):
    ...
    here). The lambda_c=0 row reuses the canonical test_result files.
      ...
      0.001     test_result{,_small}_pen0001.txt
      0.005     test_result{,_small}_pen0005.txt
      0.01      test_result{,_small}_pen001.txt    (paper default)
    ...
    for fam_label, prefix in families:
        rows = []
        for lc, tag in grid:
            fname = f"test_result{suffix}{('_' + tag) if tag else ''}.txt"
            fpath = os.path.join(SCRIPT_DIR, f"{prefix}{variant}", fname)
            data = parse_dict_result(fpath)
            if data is None:
                missing.append(fpath)
            rows.append((lc, data))
    ...
    if missing:
        print(f"\n⚠  lambda_c sweep: MISSING {len(missing)} result files. "
              f"Run CUDA_VISIBLE_DEVICES=0 bash eval_penalty_sweep_firstavg.sh. "
              f"\n   Example missing: {missing[0]}")
    ...
    out_lines = [
        r"% lambda_c hard INFERENCE-penalty sweep (same ckpt re-decoded, no retraining)",
        r"% Generated by analyze_results.py --penalty_sweep",
        r"% Variant: " + variant + r"   lambda=" + str(lamb) +
            r"   gamma_o=0 at eval   proto=" + proto,
        r"% CS@20 and MaxRun@20 are lower-is-better.",
        r"\begin{table*}[t]",
        r"\centering",
        r"\small",
        r"\caption{Sensitivity to the inference-time adjacent-penalty weight "
            r"$\lambda_c$ on \emph{" + variant.replace("kuairec_", "").replace("_", "-") +
            r"} (KuaiRec, $\lambda=" + str(lamb) + r"$, " + proto +
            r"-matrix protocol). Each row re-decodes the SAME dense checkpoint "
            r"with $\mathrm{score}(j)=(1-\lambda)\,\mathrm{rel}+\lambda\,\mathrm{div}"
            r"-\lambda_c\cos(\mathbf{v}_{j-1},\mathbf{v}_j)$; $\lambda_c$ is "
            r"independent of the training weight $\gamma_o$. $0.01$ is the default. "
            r"CS@20 is the mean adjacent-item similarity and MaxRun@20 the longest "
            r"contiguous run of mutually similar items (both lower is better). Best "
            r"per metric within each family panel is \textbf{bold}.}",
        ...
        for lc, data in rows:
            ...
            for col_idx, (mc, mkey) in enumerate(metric_cols):
                ...
                if v is not None and col_idx in best and abs(v - best[col_idx]) < 1e-12:
                    s = r"\textbf{" + s + r"}"
                cells.append(s)
            out_lines.append(f"{lc} & " + " & ".join(cells) + r" \\")
        if f_idx < len(panels) - 1:
            out_lines.append(r"\midrule")
    out_lines += [
        r"\bottomrule",
    ...
    if path is None:
        path = os.path.join(SCRIPT_DIR,
            f"penalty_sweep_{variant.replace('kuairec_','')}_{proto}.tex")
    with open(path, "w") as f:
        f.write(table_str + "\n")
    print(f"lambda_c sweep table written: {path}")

    print("\n" + "=" * 110)
    print(f"LAMBDA_C INFERENCE-PENALTY SWEEP (variant={variant}, lambda={lamb}, proto={proto})")
    print("=" * 110)
    header = f"{'family':<12} {'lambda_c':>8}  " + " ".join(f"{mc[0]:>10}" for mc in metric_cols)
    print(header)
    print("-" * len(header))
    for fam_label, rows in panels:
        for lc, data in rows:
            ...
            print(f"{fam_label:<12} {lc:>8}  {vals}")
    print("=" * 110)
...
# =============================================================================
# Singleton checkpoints — one row per PT dir for the "other datasets" table
# (gamma=1.0 single point on KuaiRec First-Average + gamma=0 best config on
# all 4 KuaiRec variants + gamma=0 best config on ML1M / KuaiRand1K / MicroLens).
# Reads test_result_gridorder.txt (big) or test_result_small_gridorder.txt
# (small) written by eval_singletons.sh.
# =============================================================================
def write_singletons_tex(proto="big"):
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    os.chdir(SCRIPT_DIR)

    # registry: (PT_DIR, display_label, dataset, gamma_label)
    SINGLES = [
        # γ=1 single-point sweep (KuaiRec First-Average)
        ("save_pt_dense_lamb001_softo1_kuairec_first_average",
         "PACER", "KuaiRec-FirstAvg", "1.0"),
        # γ=0 best config on all 4 KuaiRec variants
        ("save_pt_dense_lamb001_order0_kuairec_first_average",
         "PACER", "KuaiRec-FirstAvg",  "0"),
        ("save_pt_dense_lamb001_order0_kuairec_first_individual",
         "PACER", "KuaiRec-FirstIndv", "0"),
        ("save_pt_dense_lamb001_order0_kuairec_highest_average",
         "PACER", "KuaiRec-HighAvg",   "0"),
        ("save_pt_dense_lamb001_order0_kuairec_highest_individual",
         "PACER", "KuaiRec-HighIndv",  "0"),
        # γ=0 best config on new datasets
        ("save_pt_dense_lamb001_order0_ML1M",
         "PACER", "ML-1M",             "0"),
        ("save_pt_dense_lamb001_order0_KuaiRand1K",
         "PACER", "KuaiRand1K",        "0"),
        ("save_pt_dense_lamb001_order0_MicroLens",
         "PACER", "MicroLens",         "0"),
    ]

    # metric cols — same order as final tables
    METRIC_COLS = [
        ("HR@5",    "recall@5_f"),   ("HR@10",   "recall@10_f"),
        ("HR@20",   "recall@20_f"),  ("ND@5",    "ndcg@5_f"),
        ("ND@10",   "ndcg@10_f"),    ("ND@20",   "ndcg@20_f"),
        ("ILD@20",  "ILD@20"),       ("CC@20",   "CC@20"),
        ("CS@20",   "CS@20"),        ("MaxRun@20", "MaxRun@20"),
    ]
    ...
    for pt_dir, fam, dataset, gam in SINGLES:
        d = load_result(pt_dir, proto)
        rows.append((dataset, gam, pt_dir, d))

    # ── build LaTeX ────────────────────────────────────────────────
    header_cols = "c l c " + " ".join("c" for _ in METRIC_COLS)
    lines = []
    lines.append("%% Generated by analyze_results.py --singletons --proto %s" % proto)
    lines.append("%% Reads test_result{,_small}_gridorder.txt written by eval_singletons.sh")
    lines.append(r"\begin{table*}[t]")
    lines.append(r"\centering")
    cap = (r"Singleton checkpoints: PACER at $\gamma_o\in\{0, 1.0\}$ on all "
           "four KuaiRec variants plus three new datasets")
    lines.append(r"\caption{%s (protocol: %s).}" % (cap, proto))
    lines.append(r"\label{tab:singletons_%s}" % proto)
    lines.append(r"\vspace{1mm}")
    lines.append(r"\setlength{\tabcolsep}{4pt}")
    lines.append(r"\begin{tabular}{%s}" % header_cols)
    lines.append(r"\toprule")
    hdr = r"Dataset & Model & $\gamma_o$ & " + " & ".join(l for l, _ in METRIC_COLS) + r" \\"
    lines.append(hdr)
    lines.append(r"\midrule")

    cur_ds = None
    for dataset, gam, pt_dir, d in rows:
        if dataset != cur_ds:
            lines.append(r"\midrule")
            cur_ds = dataset
        if d is None:
            vals = " & ".join(r"\multicolumn{1}{c}{\scriptsize MISSING}" for _ in METRIC_COLS)
        else:
            vals = " & ".join(
                f"{d.get(k, float('nan')):.4f}" if d.get(k) is not None
                else r"\scriptsize MISS" for _, k in METRIC_COLS)
        lines.append(f"{dataset} & PACER & {gam} & {vals} \\\\")
    ...
    # ── write file ─────────────────────────────────────────────────
    path = os.path.join(SCRIPT_DIR,
                        f"singletons_{proto}.tex")
    with open(path, "w") as f:
        f.write(table_str + "\n")
    print(f"singletons table written: {path}")

    # ── print console summary ──────────────────────────────────────
    print("\n" + "=" * 130)
    print(f"SINGLETON CHECKPOINTS TABLE (proto={proto})")
    print("=" * 130)
    hdr = f"{'dataset':<20} {'γ':>5}  " + " ".join(f"{l:>10}" for l, _ in METRIC_COLS)
    print(hdr); print("-" * len(hdr))
    for dataset, gam, pt_dir, d in rows:
        if d is None:
            print(f"{dataset:<20} {gam:>5}  NO_RESULT  ({pt_dir})")
            continue
        vals = " ".join(f"{d.get(k, float('nan')):>10.4f}"
                        if d.get(k) is not None else f"{'MISS':>10}"
                        for _, k in METRIC_COLS)
        print(f"{dataset:<20} {gam:>5}  {vals}")
    print("=" * 130)
...
# =============================================================================
# Final paper tables: 4 tables = {KuaiRec variants, new datasets} x
# {accuracy+diversity, repetition CS/MaxRun}; columns SASRec, GRU4Rec,
# best TRIER_-t, best PACER, with best checkpoint selected by NDCG@20
# (ties: HR@20, then NDCG@10) per dataset/variant.
# =============================================================================
# families that use content (category-type) metadata -> PACER candidates
TYPE_ON_FAMILIES = {"trier_type", "trier_typeauthor", "trier_typemusic",
                    "trier_typedur", "trier_typeall"}

FINAL_ACC_METRICS = [
    ("HR@5",   "recall@5"),  ("HR@10",  "recall@10"),  ("HR@20",  "recall@20"),
    ("ND@5",   "ndcg@5"),    ("ND@10",  "ndcg@10"),    ("ND@20",  "ndcg@20"),
    ("ILD@5",  "ILD@5"),     ("ILD@10", "ILD@10"),     ("ILD@20", "ILD@20"),
    ("CC@5",   "CC@5"),      ("CC@10",  "CC@10"),      ("CC@20",  "CC@20"),
]
FINAL_REP_METRICS = [
    ("CS@5",     "CS@5"),     ("CS@10",     "CS@10"),     ("CS@20",     "CS@20"),
    ("MaxRun@5", "MaxRun@5"), ("MaxRun@10", "MaxRun@10"), ("MaxRun@20", "MaxRun@20"),
]


def _selection_score(row):
    d = row["data"]
    return (get_metric(d, "ndcg@20"), get_metric(d, "recall@20"),
            get_metric(d, "ndcg@10"))


def _pick_best(cands):
    valid = [r for r in cands
             if get_metric(r["data"], "ndcg@20") is not None]
    if not valid:
        return None
    return max(valid, key=_selection_score)


def _final_columns(big_rows, variant_key):
    """Return [(header, row_or_None), ...] for SASRec, GRU4Rec, TRIER_d, PACER."""
    vr = [r for r in big_rows if r["variant"] == variant_key]
    sas = next((r for r in vr if r["family"] == "baseline"
                and r["config"] == "sasrec"), None)
    gru = next((r for r in vr if r["family"] == "baseline"
                and r["config"] == "gru4rec"), None)
    trier_d = _pick_best([r for r in vr if r["family"] == "trier_notype"
                          and r["infer"] == "greedy"])
    pacer = _pick_best([r for r in vr if r["family"] in TYPE_ON_FAMILIES
                        and r["infer"] == "greedy"])
    return [("SASRec", sas), ("GRU4Rec", gru),
            (r"TRIER$_{d}$", trier_d), ("PACER", pacer)]


def _fmt_cell(v, rank):
    """rank: 0 bold best, 1 underline second, else plain; None -> dash."""
    if v is None:
        return "--"
    s = f"{v:.4f}"
    if rank == 0:
...
def write_final_table(big_rows, variant_labels, metrics, lower_better,
                      path, label, caption):
    """Write one final comparison table.

    variant_labels: ordered [(variant_key, display_name), ...] (multirow blocks)
    metrics:        [(display, metric_key), ...] rows inside each block
    lower_better:   set of display metric names where min wins.
    """
    out = [
        r"% Auto-generated by analyze_results.py --final",
        r"% Protocol: KuaiRec variants = small matrix (100 negatives),",
        r"% other datasets = full-catalog ranking; baselines top-k, TRIER/PACER",
        r"% step-wise greedy. Best TRIER_-t / PACER checkpoint per dataset",
        r"% chosen by NDCG@20 (ties: HR@20, then NDCG@10).",
        r"% Requires \usepackage{float} (for [H]) and \usepackage{multirow}.",
        r"\begin{table}[H]",
        r"\centering",
        ...
        r"\toprule",
        r"\textbf{Datasets} & \textbf{Metric} & \textbf{SASRec} & "
        r"\textbf{GRU4Rec} & \textbf{TRIER$_{d}$} & \textbf{PACER} \\",
        r"\midrule",
    ]

    n_metrics = len(metrics)
    for block_idx, (vkey, vlabel) in enumerate(variant_labels):
        cols = _final_columns(big_rows, vkey)
        for i, (mdisp, mkey) in enumerate(metrics):
            vals = [get_metric(r["data"], mkey) if r else None
                    for _, r in cols]
            present = sorted([v for v in vals if v is not None],
                             reverse=(mdisp not in lower_better))
            ranks = {}
            for rank, v in enumerate(present[:2]):
                ranks.setdefault(v, rank)
            cells = []
            for v in vals:
                cells.append(_fmt_cell(v, ranks.get(v, -1)) if v is not None
                             else "--")
            if i == 0:
                out.append(rf"\multirow{{{n_metrics}}}{{*}}{{{vlabel}}} & "
                           + f"{mdisp} & " + " & ".join(cells) + r" \\")
            else:
                out.append(f" & {mdisp} & " + " & ".join(cells) + r" \\")
        if block_idx != len(variant_labels) - 1:
            out.append(r"\midrule")
    out += [
        r"\bottomrule",
        r"\end{tabular}",
        r"\captionsetup{font=scriptsize}",
        caption,
        rf"\label{{{label}}}",
        r"\end{table}",
        "",
    ...
    with open(path, "w") as f:
        f.write("\n".join(out))
    print(f"Final table written: {path}")


def write_final_tables():
    """Collect per-group-protocol rows and emit 4 tables.

    Protocol differs by dataset group BY DESIGN: the four KuaiRec variants
    use the small-matrix protocol (100 negatives), while MicroLens/ML-1M/
    KuaiRand use full-catalog (big) ranking.
    """
    kuai_keys = [v[0] for v in DATASETS["kuairec"]["variants"]]
    newds_keys = ["MicroLens", "ML1M", "KuaiRand1K"]
    all_rows = collect_all_results(variant_keys=kuai_keys + newds_keys)
    kuai_rows = [r for r in all_rows if r["variant"] in kuai_keys
                 and r["proto"] == "small"]
    newds_rows = [r for r in all_rows if r["variant"] in newds_keys
                  and r["proto"] == "big"]
    if not kuai_rows and not newds_rows:
        print("No results found; skipping final tables.")
        return

    kuai_labels = DATASETS["kuairec"]["variants"]
    newds_labels = [("MicroLens", "MicroLens"), ("ML1M", "ML-1M"),
                    ("KuaiRand1K", "KuaiRand")]

    cap_select = (r"For each dataset the best TRIER$_{-t}$ checkpoint (no "
        r"content metadata) and the best content-enriched PACER checkpoint are "
        r"selected by NDCG@20 on that dataset (ties broken by HR@20, then "
        r"NDCG@10). Best result per row is in \textbf{bold}, second-best in "
        r"\underline{underline}.")
    cap_proto_small = (r"under the small-matrix protocol (100 sampled "
        r"negatives). ")
    cap_proto_big = (r"under the step-wise greedy decoding protocol "
        r"(full-catalog ranking). ")
    cap_acc_dir = (r"Higher HR/NDCG/ILD/CC is better.")
    cap_rep_dir = (r"CS@$K$ (average consecutive-item similarity) and "
        r"MaxRun@$K$ (longest contiguous run of mutually similar items) are "
        r"both lower-better diversity metrics.")
    rep_lower = {"CS@5", "CS@10", "CS@20",
                 "MaxRun@5", "MaxRun@10", "MaxRun@20"}

    # --- KuaiRec variants (small-matrix protocol) ---
    if kuai_rows:
        write_final_table(
            kuai_rows, kuai_labels, FINAL_ACC_METRICS, set(),
            os.path.join(SCRIPT_DIR, "final_kuairec_accdiv.tex"),
            "tab:final_kuairec_acc",
            r"\caption{Overall comparison across the four KuaiRec variants "
            + cap_proto_small + cap_select + " " + cap_acc_dir + "}")
        write_final_table(
            kuai_rows, kuai_labels, FINAL_REP_METRICS, rep_lower,
            os.path.join(SCRIPT_DIR, "final_kuairec_rep.tex"),
            "tab:final_kuairec_rep",
            r"\caption{Repetition metrics across the four KuaiRec variants "
            + cap_proto_small + cap_select + " " + cap_rep_dir + "}")

    # --- New datasets (full-catalog / big protocol) ---
    if newds_rows:
        write_final_table(
            newds_rows, newds_labels, FINAL_ACC_METRICS, set(),
            os.path.join(SCRIPT_DIR, "final_newds_accdiv.tex"),
            "tab:final_newds_acc",
            r"\caption{Overall comparison across MicroLens, ML-1M, and KuaiRand "
            + cap_proto_big + cap_select + " " + cap_acc_dir + "}")
        write_final_table(
            newds_rows, newds_labels, FINAL_REP_METRICS, rep_lower,
            os.path.join(SCRIPT_DIR, "final_newds_rep.tex"),
            "tab:final_newds_rep",
            r"\caption{Repetition metrics across MicroLens, ML-1M, and KuaiRand "
            + cap_proto_big + cap_select + " " + cap_rep_dir + "}")
...
def main():
    ...
    parser.add_argument("--T_sweep", action="store_true",
                        help="Additionally generate the T soft-selection "
                             "temperature sweep table for the fixed L_order "
                             "(reads test_result{,_small}_softoT_*.txt populated "
                             "by eval_softoT_grid_firstavg.sh)")
    parser.add_argument("--singletons", action="store_true",
                        help="Single-row-per-checkpoint table for γ=1 KuaiRec + "
                             "all KuaiRec variants γ=0 + new datasets γ=0 "
                             "(runs after eval_singletons.sh)")
    parser.add_argument("--penalty_sweep", action="store_true",
                        help="Additionally generate the lambda_c hard "
                             "inference-penalty sweep table (same checkpoints "
                             "re-decoded; reads test_result{,_small}_pen*.txt "
                             "populated by eval_penalty_sweep_firstavg.sh)")
    parser.add_argument("--final", action="store_true",
                        help="Generate the four final paper tables "
                             "(final_{kuairec,newds}_{accdiv,rep}.tex): KuaiRec "
                             "variants use the small-matrix protocol, other "
                             "datasets full-catalog ranking; SASRec/GRU4Rec + "
                             "best TRIER_d/PACER by NDCG@20")
    ...
    if not rows and not (args.sixcell or args.embedding_ablation
                         ...
                         or args.T_sweep or args.penalty_sweep
                         or args.singletons or args.final):
    ...
    # --- gamma_o adjacent-order training-weight sweep ---
    if args.gamma_sweep:
        for proto in protos:
    ...
    # --- T soft-selection temperature sweep (fixed L_order) ---
    if args.T_sweep:
        for proto in protos:
            write_softoT_sweep_tex(
                variant="kuairec_first_average",
                gamma=0.01,
                lamb=0.01,
                proto=proto,
    ...
    # --- lambda_c hard inference-penalty sweep (same ckpt, re-decoded) ---
    if args.penalty_sweep:
        for proto in protos:
            write_penalty_sweep_tex(
                variant="kuairec_first_average",
                lamb=0.01,
                proto=proto,
            )
    ...
    if args.singletons:
        for proto in protos:
            write_singletons_tex(proto=proto)

    # --- four final paper tables (both dataset groups, big protocol) ---
    if args.final:
        write_final_tables()
