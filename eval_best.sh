...
for VAR in "${VARIANTS[@]}"; do
    ...
    # Reads Python dict lines, finds epoch with highest recall@5 (PT) or recall@5 (RT)
    get_best_val_epoch() {
        local valid_file="$1"
        python3 -c "
import ast, sys
...
with open('$valid_file') as f:
    for line in f:
        ...
        r5 = d.get('recall@5_f', d.get('recall@5', 0))
        if r5 > best_r5:
            best_r5 = r5
            best_ep = d['epoch']
print(best_ep)
"
    ...
    if [ -n "$LATEST_RT" ]; then
        ...
        # Phase 1: Validate every 10th epoch
        ...
        # Phase 2: Find best validation epoch
