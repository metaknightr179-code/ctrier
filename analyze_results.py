...
def tex_model_label(fam, cfg, infer):
    """Compact paper-style model label for LaTeX tables."""
    if fam == "baseline":
        ...
        base = "GRU-TRIER " + CONFIG_LABELS.get(cfg, cfg)
    else:
        # TRIER with type embeddings is the full model (paper's "TRIER");
        # the no-type ablation gets a subscript to keep labels short.
        prefix = "TRIER " if fam == "trier_type" else r"TRIER$_{-t}$ "
        base = prefix + CONFIG_LABELS.get(cfg, cfg)
    if fam not in ("baseline", "duorec"):
        base += " (" + infer + ")"
    return base

def write_latex(rows, path, ds_label="", suffix=""):
    # Metric columns mirror the TRIER paper's main table: accuracy (R, N)
    # + diversity (ILD, CC).
    ...
    lines = [r"% Auto-generated by analyze_results.py",
             r"% Requires packages: booktabs, multirow, graphicx",
             ""]
    for proto in ("big", "small"):
        ...
        # best (max) value per (variant, metric) column for bolding
        best = {}
        for vi, (vk, _) in enumerate(VARIANTS):
            for ci, (_, m) in enumerate(cols):
                vals = [get_metric(vd.get(vk), m) for vd in models.values()]
                vals = [v for v in vals if v is not None]
                if vals:
                    best[(vi, ci)] = max(vals)

        # Wide tables (>=3 dataset groups) span both columns and are scaled to
        # \textwidth; narrow tables fit a single column at footnotesize.
        wide = len(VARIANTS) >= 3
        ...
        lines.append(r"\caption{Overall comparison on " + cap_ds + r" ("
                     ...
                     + r"). Best result in each column is in \textbf{bold}.}")
        ...
        for i, (_, vl) in enumerate(VARIANTS):
            h1 += r" & \multicolumn{" + str(len(cols)) + r"}{c}{" \
                  + vl.replace("kuairec_", "").replace("_", "-") + r"}"
        h1 += r" \\"
        lines.append(h1)
        cmid = " ".join(
            r"\cmidrule(lr){" + str(2 + i * len(cols)) + "-"
            + str(1 + (i + 1) * len(cols)) + "}"
        ...
        for (fam, cfg, infer), var_data in models.items():
            row = tex_model_label(fam, cfg, infer)
            for vi, (vk, _) in enumerate(VARIANTS):
                ...
                for ci, (_, m) in enumerate(cols):
                    ...
                    if v is None:
                        ...
                        cell = f"{v:.4f}"
                        if abs(v - best.get((vi, ci), v)) < 1e-9:
                            cell = r"\textbf{" + cell + "}"
                    row += " & " + cell
            row += r" \\"
            lines.append(row)
