...
def get_args():
    ...
    parser.add_argument('-tf', type=str, default='./Yelp/train-v0.txt', help='训练集数据的路径')
    parser.add_argument('-vf', type=str, default='./Yelp/valid-v0.txt', help='验证集数据的路径')
    ...
    parser.add_argument('-n', type=int, default=14588, help='商品数量')  # Yelp数据集item数量
    ...
    parser.add_argument('-lmd_consec', type=float, default=0.0, help='consecutive-similarity penalty weight in the generation score (default 0 = off)')
    parser.add_argument('-tau_o', type=float, default=0.1, help='temperature of the prospective intent softmax P_a(j); sharpness multiplier is 1/tau_o (default 0.1 -> x10, original TRIER behavior)')
    parser.add_argument('-soft_order_loss', action='store_true', help='use the differentiable SOFT order loss L_order (soft item selection) instead of the hard argmax L_consec')
    parser.add_argument('-lmd_softorder', type=float, default=0.01, help='weight of the differentiable soft order loss (used with -soft_order_loss)')
    parser.add_argument('-soft_order_temp', type=float, default=1.0, help='softmax temperature for soft item selection inside L_order')
    parser.add_argument('-gamma_consec', type=float, default=0.01, help='consecutive similarity loss weight (default 0.01)')
