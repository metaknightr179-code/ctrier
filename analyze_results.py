...
def write_final_table(big_rows, variant_labels, metrics, lower_better,
    ...
    out = [
        ...
        r"% Protocol: big matrix, full-catalog ranking; baselines top-k,",
        r"% TRIER/PACER step-wise greedy. Best TRIER_-t / PACER checkpoint per",
        r"% dataset chosen by NDCG@20 (ties: HR@20, then NDCG@10).",
...
def write_final_tables():
    """Collect big-protocol rows for both dataset groups and emit 4 tables."""
    kuai_keys = [v[0] for v in DATASETS["kuairec"]["variants"]]
    newds_keys = ["MicroLens", "ML1M", "KuaiRand1K"]
    all_keys = kuai_keys + newds_keys
    big_rows = [r for r in collect_all_results(variant_keys=all_keys)
                if r["proto"] == "big"]
    if not big_rows:
        print("No big-protocol results found; skipping final tables.")
        return
    ...
    cap_proto = (r"under the step-wise greedy decoding protocol (full-catalog "
        r"ranking). ")
    ...
    # --- KuaiRec variants ---
    if any(r["variant"] in kuai_keys for r in big_rows):
        write_final_table(
            big_rows, kuai_labels, FINAL_ACC_METRICS, set(),
            ...
            + cap_proto + cap_select + " " + cap_acc_dir + "}")
        write_final_table(
            big_rows, kuai_labels, FINAL_REP_METRICS,
            {"CS@5", "CS@10", "CS@20", "MaxRun@5", "MaxRun@10", "MaxRun@20"},
            ...
            + cap_proto + cap_select + " " + cap_rep_dir + "}")

    # --- New datasets ---
    if any(r["variant"] in newds_keys for r in big_rows):
        write_final_table(
            big_rows, newds_labels, FINAL_ACC_METRICS, set(),
            os.path.join(SCRIPT_DIR, "final_newds_accdiv.tex"),
            "tab:final_newds_acc",
            r"\caption{Overall comparison across MicroLens, ML-1M, and KuaiRand "
            + cap_proto + cap_select + " " + cap_acc_dir + "}")
        write_final_table(
            big_rows, newds_labels, FINAL_REP_METRICS,
            {"CS@5", "CS@10", "CS@20", "MaxRun@5", "MaxRun@10", "MaxRun@20"},
...
def main():
    ...
    parser.add_argument("--final", action="store_true",
                        help="Generate the four final paper tables "
                             "(final_{kuairec,newds}_{accdiv,rep}.tex): big "
                             "matrix, SASRec/GRU4Rec + best TRIER_d/PACER by "
                             "NDCG@20, across both dataset groups")
