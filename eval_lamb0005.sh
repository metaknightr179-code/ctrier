#!/bin/bash
# =============================================================================
# Eval PT models trained with lamb=0.005 (latest checkpoint only)
...
echo "=============================================="
echo "Evaluating PT (lamb=0.005) models"
echo "=============================================="

for VAR in "${VARIANTS[@]}"; do
    RT_DIR="./save_rt_${VAR}"
    PT_DIR="./save_pt_lamb0005_${VAR}"
    ...
    # Check if PT lamb=0.005 model exists
    LATEST_PT=$(get_latest_epoch "${PT_DIR}/model")
    if [ -z "$LATEST_PT" ]; then
        echo "[PT lamb=0.005] No checkpoints found — skipping (not trained yet)"
        continue
    fi
    ...
    echo "[PT lamb=0.005] Evaluating epoch ${LATEST_PT}..."
    ...
    CUDA_VISIBLE_DEVICES=${GPU} python3 main_pt.py \
        ...
        -div -lamb 0.005 -t_mode topk \
        -start_epoch ${LATEST_PT} -epoch_step 1 \
        -i ${RT_DIR} -o ${PT_DIR} 2>&1 | tee eval_pt_lamb0005_valid_${VAR}.log
    ...
    CUDA_VISIBLE_DEVICES=${GPU} python3 main_pt.py \
        ...
        -div -lamb 0.005 -t_mode topk \
        -start_epoch ${LATEST_PT} -epoch_step 1 \
        -i ${RT_DIR} -o ${PT_DIR} 2>&1 | tee eval_pt_lamb0005_test_${VAR}.log
...
echo ""
echo "=============================================="
echo "ALL lamb=0.005 EVALUATIONS COMPLETE!"
echo "=============================================="
