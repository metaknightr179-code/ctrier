#!/bin/bash
# =============================================================================
# Dense PT training — NO-TYPE family only
# Run in parallel with train_dense_type_fixrt.sh on the same GPU
# Usage: nohup bash train_dense_notype_fixrt.sh <GPU_ID> > train_dense_notype.log 2>&1 &
# =============================================================================
...
echo "############################################################"
echo "# Dense PT [NO-TYPE] training, GPU ${GPU}"
echo "############################################################"
...
for CFG in "${CONFIGS[@]}"; do
    ...
    for VAR in "${VARIANTS[@]}"; do
        pt_dir="save_pt_notype_dense_${CFG_SUFFIX}_${VAR}"
        pt_log="pt_dense_${CFG_SUFFIX}_notype_${VAR}.log"
        rt_dir="save_rt_fix_${VAR}"

        echo "============================================================"
        echo "PT Dense [NO-TYPE]: ${CFG_NAME} / ${VAR} -> ${pt_dir}"
        echo "============================================================"
        ...
        # deletes lowest-numbered). Reconcile log lines to the newest checkpoint.
        RESUME=""
        ...
        CUDA_VISIBLE_DEVICES=${GPU} python3 main_pt.py \
            ...
            -dense -no_type -t_mode topk -early_stop -patience 100 -min_delta 0.0001 \
            ${RESUME} ${CFG_FLAGS} \
            -i ./${rt_dir} -o ./${pt_dir} 2>&1 | tee "${pt_log}"
    done
done

echo "DENSE PT [NO-TYPE] TRAINING COMPLETE"
