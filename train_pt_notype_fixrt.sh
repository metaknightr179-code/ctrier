#!/bin/bash
# =============================================================================
# Stage 3 of train_pipeline_fixrt.sh extracted for PARALLEL GPU execution:
# PT models WITHOUT type embeddings (-no_type, diversity sweep).
#
# Usage:
#   nohup bash train_pt_notype_fixrt.sh <GPU_ID> > pt_notype_1000.log 2>&1 &
#   e.g.: nohup bash train_pt_notype_fixrt.sh 1 > pt_notype_1000.log 2>&1 &
#
...
# train_pt_type_fixrt.sh with a different GPU ID at the same time to
...
echo "############################################################"
echo "# PT [NO-TYPE] stage (without type embeddings), GPU ${GPU}"
...
echo "PT [NO-TYPE] STAGE COMPLETE"
